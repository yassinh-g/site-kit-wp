...
export async function getAdSenseAccountStatus( statusUpdateCallback, existingTag = false ) {
	...
	try {
		...
		if ( ! accountData || ! id || hasError ) {
			...
			// If multiple accounts are returned, we need to search through all
			// accounts to find any accounts with matching domains.
			...
			if ( find( alertsResults, { type: 'GRAYLISTED_PUBLISHER' } ) ) {
				...
				if ( hasAlertsError ) {
					...
					if ( hasClientError ) {
						...
						const urlchannels = await data.get( TYPE_MODULES, 'adsense', 'urlchannels', { clientId } )
							.then( ( res ) => res )
							.catch( ( e ) => e );

						// Find a URL channel with a matching domain
						const matches = (
							urlchannels && urlchannels.length &&
							urlchannels.filter( ( channel ) => {
								return -1 === googlesitekit.admin.siteURL.indexOf(
									channel.urlPattern
								);
							} )
						);
...
export const isDataZeroAdSense = ( adSenseData, datapoint, dataRequest ) => {
	...
	if ( ! dataRequest.data || ! dataRequest.data.date_range || 'last-28-days' !== dataRequest.data.date_range ) {
		return false;
	}
